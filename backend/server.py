...
# ---------------------------------------------------------------------------
# Install & Gateway Endpoints
# ---------------------------------------------------------------------------

@app.get("/api/hermes/install-cmd")
async def get_hermes_install_cmd():
    """Returns install instructions for local CLI and Docker modes."""
    return {
        "cli": "curl -fsSL https://raw.githubusercontent.com/NousResearch/hermes-agent/main/scripts/install.sh | bash",
        "docker": "docker pull nousresearch/hermes-agent:v0.5.0\ndocker run -it nousresearch/hermes-agent:v0.5.0" # Placeholder, Docker requires actual compose config but simple pull instruction serves here.
    }

@app.get("/api/gateway/status")
async def get_gateway_status():
    """Proxy to hermes gateway status."""
    hermes_cmd = get_hermes_cmd()
    try:
        proc = await asyncio.create_subprocess_exec(
        ...
        # Parse standard output for "Running: " or similar platform statuses
        platforms = {
            "telegram": False,
            "discord": False,
            "whatsapp": False,
        ...
        if "telegram: running" in out_lower or "telegram is running" in out_lower or "telegram [running]" in out_lower:
            platforms["telegram"] = True
        if "discord: running" in out_lower or "discord is running" in out_lower or "discord [running]" in out_lower:
            platforms["discord"] = True
        if "whatsapp: running" in out_lower or "whatsapp is running" in out_lower or "whatsapp [running]" in out_lower:
            platforms["whatsapp"] = True
            
        return {"status": "ok", "platforms": platforms, "raw": output}
    except Exception as e:
        return {"status": "error", "platforms": {}, "raw": str(e)}
