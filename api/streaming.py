    ...
    try:
        ...
        with _ENV_LOCK:
          ...
          try:
            ...
            if AIAgent is None:
                raise ImportError(f"AIAgent not available -- check that hermes-agent is on sys.path. AIAgent is {AIAgent}")
            resolved_model, resolved_provider, resolved_base_url = resolve_model_provider(model)
            if not resolved_model:
                raise ValueError("Model ID is empty. Please select a model in the sidebar.")
        ...
        # Delay cleanup to allow client EventSource to connect and receive any fast-failure events
