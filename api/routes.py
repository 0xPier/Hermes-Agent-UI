...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path in ('/', '/index.html'):
        ...
        # Detect if any model configuration exists
        needs_setup = not config_path.exists()
        if not needs_setup:
            try:
                content = config_path.read_text().lower()
                # If no model and no custom providers, jump to wizard
                if 'model:' not in content and 'custom_providers:' not in content:
                    needs_setup = True
            except:
                needs_setup = True
        ...
        if needs_setup:
            ...
            wizard_path = (Path(__file__).parent.parent / 'static' / 'wizard.html').resolve()
            return t(handler, wizard_path.read_text(encoding='utf-8'),
    ...
    if parsed.path == '/api/health':
        ...
        if not shutil.which('hermes'):
            installed = False
        return j(handler, {'installed': installed})
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=5) as res:
                if res.status == 200:
                    ...
                    return j(handler, {'status': 'ok', 'reachable': True, 'models': models})
                return j(handler, {'status': 'error', 'reachable': True, 'models': [], 'message': f"HTTP {res.status}"})
        except Exception as e:
            return j(handler, {'status': 'error', 'reachable': False, 'models': [], 'message': str(e)})
