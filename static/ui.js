...
// Finds a model Id in the suggestions datalist – kept for backward compatibility but less critical now.
function _findModelInSuggestions(modelId){
  if(!modelId) return null;
  const dl=$('modelSuggestions');
  if(!dl) return modelId;
  const opts=Array.from(dl.options).map(o=>o.value);
  if(opts.includes(modelId)) return modelId;
  return modelId; // Just return what we have if it's a text input
}

function _applyModelToInput(modelId){
  const el=$('modelSelect');
  if(!el || !modelId) return;
  el.value=modelId;
  const chip=$('modelChip');
  if(chip) chip.textContent=getModelLabel(modelId);
}

async function populateModelSuggestions(){
  const dl=$('modelSuggestions');
  if(!dl) return;
  try{
    ...
    if(data.default_model && !localStorage.getItem('hermes-webui-model')){
      _applyModelToInput(data.default_model);
    }
  }catch(e){
...
function syncTopbar(){
  ...
  if(modelOverride){
    ...
    _applyModelToInput(modelOverride);
  } else {
    const m=S.session.model||'';
    if(m) _applyModelToInput(m);
  }
